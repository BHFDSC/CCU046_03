#
# Derive analysis dataset for stroke
#
# K. Fleetwood
...
cohort_tbl %>%
  ...
  mutate(
    ...
    max_age = 
      case_when(
        is.na(out_stroke_primary_date) & is.na(date_of_death) ~ age_end_fu,
        !is.na(out_stroke_primary_date) ~ age_at_strk,
...
for (i in 1:50){
  ...
  month_i_sum <- 
    ...
    filter(
      ...
      is.na(out_stroke_primary_date)|(out_stroke_primary_date >= month_start_i),
    ...
    mutate(
      ...
      person_years_end = pmin(month_end, date_of_death, out_stroke_primary_date),
      person_years_total = DATEDIFF(sql("DAY"), person_years_start, person_years_end) + 1,
      smi_strk_date = 
        case_when(
          smi_strk %in% "No SMI" ~ NA,
          smi_strk %in% "Schizophrenia"~ sch_date_strk,
          smi_strk %in% "Bipolar disorder" ~ bd_date_strk,
          smi_strk %in% "Depression" ~ dep_date_strk
        ),
      smi_start = pmax(month_start, date_of_birth, smi_strk_date),
      person_years_smi = 
        case_when(
      ...
      person_years_no_smi = 
        case_when(
          smi_strk %in% "No SMI" ~ person_years_total,
          TRUE ~ person_years_total - person_years_smi
        ),
      person_years_sch = 
        case_when(
          smi_strk %in% "Schizophrenia" ~ person_years_smi,
          TRUE ~ 0
        ),
      person_years_bd = 
        case_when(
          smi_strk %in% "Bipolar disorder" ~ person_years_smi,
          TRUE ~ 0
        ),
      person_years_dep = 
        case_when(
          smi_strk %in% "Depression" ~ person_years_smi,
          TRUE ~ 0
        ),
      ...
      events_total = 
        case_when(
          !is.na(out_stroke_primary_date) & out_stroke_primary_date <= month_end & out_stroke_primary_date >= date_of_birth ~ 1,
          TRUE                                                             ~ 0
        ),
      events_smi = 
        case_when(
          !is.na(out_stroke_primary_date) & out_stroke_primary_date <= month_end & out_stroke_primary_date > smi_strk_date & out_stroke_primary_date >= date_of_birth ~ 1,
          TRUE                                                                                                   ~ 0
        ),
      events_no_smi = 
        case_when(
          smi_strk %in% "No SMI" ~ events_total,
          TRUE                 ~ events_total - events_smi
        ),
      events_sch = 
        case_when(
          smi_strk %in% "Schizophrenia" ~ events_smi,
          TRUE ~ 0
        ),
      events_bd = 
        case_when(
          smi_strk %in% "Bipolar disorder" ~ events_smi,
          TRUE ~ 0
        ),
      events_dep = 
        case_when(
          smi_strk %in% "Depression" ~ events_smi,
          TRUE ~ 0
        )
    ) %>%
    group_by(
...
saveRDS(month_sum_long, file = file.path(processed_data_folder, "02_stroke_dataset.rds"))  
