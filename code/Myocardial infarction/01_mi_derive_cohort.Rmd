---
title: 'Myocardial infarction: derive cohort'
author: "K. Fleetwood"
date: "`r Sys.Date()`"
output:
...
# 18 Nov: Updated to include people under 40
...
This document describes the derivation of the cohort for CCU046_03, analysis of myocardial infarction (MI) incidence. 
...
# - Exclude people with an MI prior to the start of the study period
gdppr_n_wo_mi <-
  cohort_tbl %>%
    filter(
      in_gdppr %in% 1,
    ...
    summarise(n())
gdppr_n_wo_mi <- as.numeric(as.data.frame(gdppr_n_wo_mi)[1,1])
n_ex_mi <- gdppr_n  - gdppr_n_wo_mi

# Exclude people with an event date incompatible with their date of death
...
n_ex_datebad <- gdppr_n_wo_mi - gdppr_n_dateok

# For people in the cohort for the age-standardised analyses, calculate the number with an mi
mi_count_as <- 
  cohort_tbl %>%
  filter(
    in_gdppr %in% 1,
    ...
    is.na(date_of_death)|is.na(out_ami_primary_date)|(date_of_death >= out_ami_primary_date),
    is.na(out_ami_primary_date)|(out_ami_primary_date >= as.Date("2019-11-01")),
    out_ami_primary_flag == 1
  ) %>%
  summarise(n())

mi_count_as <- as.numeric(as.data.frame(mi_count_as)[1,1])
mi_per_as <- round(100*mi_count_as/gdppr_n_dateok, 1)
...
gdppr_n_wo_mi_40 <-
  cohort_tbl %>%
    ...
    summarise(n())
gdppr_n_wo_mi_40 <- as.numeric(as.data.frame(gdppr_n_wo_mi_40)[1,1])
n_ex_40 <- gdppr_n_wo_mi - gdppr_n_wo_mi_40
...
n_ex_100 <- gdppr_n_wo_mi_40 - gpppr_n_40_100
...
mi_count <- as.numeric(as.data.frame(mi_count)[1,1])
```
...
* `r format(rnd5(n_ex_mi), digits = 10, big.mark = ",")` people excluded due to MI prior to start of follow-up
* `r format(rnd5(gdppr_n_wo_mi), digits = 10, big.mark = ",")` people remaining in the cohort
* `r format(rnd5(n_ex_datebad), digits = 10, big.mark = ",")` people excluded due to date of death prior to date of MI
* `r format(rnd5(gdppr_n_dateok), digits = 10, big.mark = ",")` included in the cohort for the age-standardised incidence analyses
* Of the people included in the cohort for the age-standardised incidence analyses, `r format(rnd5(mi_count_as), digits = 10, big.mark = ",")` (`r mi_per_as`%) had an MI during the follow-up period 
...
# Extract data for people with an MI
cohort_mi_as <- 
  cohort_tbl %>%
  filter(
    in_gdppr %in% 1,
  ...
  select(
    ...
    primary_exp_smi_label, primary_exp_smi_date, out_ami_primary_date
  ) %>%
  collect()

# Need to correct the SMI label to exclude events after the date of MI
cohort_mi_as <- 
  cohort_mi_as %>%
  mutate(
    sch_date_mi = 
    ...
    smi_mi = factor(smi_mi, levels = c("Schizophrenia", "Bipolar disorder", "Depression", "No SMI")),
    smi_mi_date = 
      case_when(
        smi_mi == "Schizophrenia" ~ primary_exp_schizophrenia_date,
        smi_mi == "Bipolar disorder" ~ primary_exp_bipolar_disorder_date,
        smi_mi == "Depression" ~ primary_exp_depression_date,
        smi_mi == "No SMI" ~ NA,
      )
  )
...
cohort_mi_as <- 
  cohort_mi_as %>%
  mutate(
    age_at_mi = as.numeric(out_ami_primary_date - date_of_birth)/365.25,
    sex_lab = 
      case_when(
        sex %in% "F" ~ "Female",
        sex %in% "M" ~ "Male"
...
bc_tab_as <- 
  tableby(
    smi_mi ~ age_at_mi + sex_lab + includeNA(imd_quintile_lab, first = FALSE) + includeNA(ethnic_5, first = FALSE),
    data = cohort_mi_as,
    control = tab_cntrls, 
    digits = 1, 
    stats.labels = 
...
bc_tab_as$tables$smi_mi$y$stats <- 5*round(bc_tab_as$tables$smi_mi$y$stats/5)

# Repeat for men
bc_tab_m_as <- 
  tableby(
    smi_mi ~ age_at_mi + includeNA(imd_quintile_lab, first = FALSE) + includeNA(ethnic_5, first = FALSE),
    data = filter(cohort_mi_as, sex == "M"),
    control = tab_cntrls, 
    digits = 1, 
    stats.labels = 
      list(meansd = 'Mean (SD)',
...
bc_tab_m_as$tables$smi_mi$y$stats <- 5*round(bc_tab_m_as$tables$smi_mi$y$stats/5)

# Repeat for women
bc_tab_f_as <- 
  tableby(
    smi_mi ~ age_at_mi + includeNA(imd_quintile_lab, first = FALSE) + includeNA(ethnic_5, first = FALSE),
    data = filter(cohort_mi_as, sex == "F"),
    control = tab_cntrls, 
    digits = 1, 
    stats.labels = 
      list(meansd = 'Mean (SD)',
...
bc_tab_f_as$tables$smi_mi$y$stats <- 5*round(bc_tab_f_as$tables$smi_mi$y$stats/5)
```
...
cohort_mi_40 <- cohort_mi %>% filter(age_at_mi >= 40)
ex_under_40 <- mi_count - nrow(cohort_mi_40)

# Additionally exclude people with MI aged greater than or equal to 101
cohort_gt_100 <- 
  sum(cohort_mi_40$age_at_mi >= 101)

cohort_mi_40 <- cohort_mi_40 %>% filter(age_at_mi < 101)
...
bc_tab$tables$smi_mi$y$stats <- 5*round(bc_tab$tables$smi_mi$y$stats/5)

# Repeat for men
bc_tab_m <- 
  tableby(
    smi_mi ~ age_at_mi + includeNA(imd_quintile_lab, first = FALSE) + includeNA(ethnic_5, first = FALSE),
    data = filter(cohort_mi_40, sex == "M"),
    control = tab_cntrls, 
    digits = 1, 
    stats.labels = 
      list(meansd = 'Mean (SD)',
...
bc_tab_m$tables$smi_mi$y$stats <- 5*round(bc_tab_m$tables$smi_mi$y$stats/5)

# Repeat for women
bc_tab_f <- 
  tableby(
    smi_mi ~ age_at_mi + includeNA(imd_quintile_lab, first = FALSE) + includeNA(ethnic_5, first = FALSE),
    data = filter(cohort_mi_40, sex == "F"),
    control = tab_cntrls, 
    digits = 1, 
    stats.labels = 
      list(meansd = 'Mean (SD)',
...
bc_tab_f$tables$smi_mi$y$stats <- 5*round(bc_tab_f$tables$smi_mi$y$stats/5)

# Count of people excluded due to an MI during the follow-up period, before the age of 40
cohort_n <- gpppr_n_40_100 - ex_under_40
mi_per <- round(100*nrow(cohort_mi_40)/cohort_n, 1) 
```
* Of those remaining, `r format(rnd5(ex_under_40), digits = 10, big.mark = ",")` excluded due to MI before age 40
* `r format(rnd5(cohort_n), digits = 10, big.mark = ",")` people included in cohort
* `r format(rnd5(nrow(cohort_mi_40)), digits = 10, big.mark = ",")` (`r mi_per`%) had an MI during the follow-up period,  aged between 40 and 100 inclusive

# Baseline characteristics of people with an MI (cohort for age-standardised incidence rates)
```{r by_smi_as, results='asis'}

arsenal:::summary.tableby(
  bc_tab_as, 
  title = "Baseline characteristics of people with an MI"
)

arsenal:::summary.tableby(
  bc_tab_m_as, 
  title = "Baseline characteristics of men with an MI"
)

arsenal:::summary.tableby(
  bc_tab_f_as, 
  title = "Baseline characteristics of women with an MI"
)
```

# Baseline characteristics of people with an MI (cohort for Poisson modelling)
...
# Summarise missing ethnicity and deprivation by SMI and MI status -------------

# Select cohort (exclude people with an MI before their 40th birthday)
# Redefine SMI
...
cohort_tbl %>%
  ...
  mutate(
    ...
    mi = 
      case_when(
        out_ami_primary_flag %in% 1 ~ "MI",
        is.na(out_ami_primary_flag) ~ "No MI"
      ),
...
cohort_miss_sum %>%
  mutate(
    smi_mi = factor(smi_mi, levels = smi_lvls),
...
# Present: age, sex, deprivation, smi status, mi status
cohort_miss_by_eth <-
  cohort_tbl %>%
    mutate(
    age_at_mi  = DATEDIFF(sql("DAY"), date_of_birth, out_ami_primary_date),
  ...
  summarise(
    ...
    smi_sch    = sum(as.numeric(smi_mi %in% "Schizophrenia")),
    smi_bd     = sum(as.numeric(smi_mi %in% "Bipolar disorder")),
    smi_dep    = sum(as.numeric(smi_mi %in% "Depression")), 
    smi_no     = sum(as.numeric(smi_mi %in% "No SMI")),
    mi_yes     = sum(as.numeric(mi %in% "MI")),
    mi_no      = sum(as.numeric(mi %in% "No MI"))
  ) %>%
  collect()
...
cohort_miss_by_eth <- 
  ...
  mutate(
    ...
    mi_yes_fmt     = n_per_fmt(mi_yes, n),
    mi_no_fmt      = n_per_fmt(mi_no, n)
  )
...
rownames(cohort_miss_by_eth_out) <- 
  c("N", "Age at MI (years), [Mean (SD)]", "Sex, Male", "Sex, Female", 
...
# Present: age, sex, ethnicity, smi status, mi status
...
# Present age, sex, smi status, mi_status
...
kable(
  select(cohort_miss_sum_out, smi_mi, mi, n_rnd, miss_eth_out, miss_dep_out, miss_any_out),
  caption = "Missing data by SMI and MI status",
  col.names = c("SMI", "MI", "N", "Missing ethnicity, n (%)", "Missing deprivation, n (%)", "Missing any, n (%)")
)
