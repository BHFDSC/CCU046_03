#
# Age-standardised incidence rates for myocardial infarction
#
# K. Fleetwood
# 1 Aug 2024:  date started
# 24 Nov 2024: updated to include people younger than 40 and older than 100
...
pwd <- 
  readLines("D:\\PhotonUser\\My Files\\Home Folder\\ccu046_db_token_for_r_kf.txt")
...
# Read in primary mi analysis dataset
mi_data <- readRDS(file = file.path(processed_data_folder, "mi_dataset_23Nov2024.rds"))  
...
esp <- 
  esp %>% 
  filter(
    age_group %in% pull(mi_data, age_group)
  ) %>%
  mutate(
...
# 3.2 Define data for monthly incidence by sex (not by SMI status) ------------
...
# 3.5 Define data for 4-monthly incidence by SMI status (not by sex) -----------
...
month_starts <- unique(mi_data_grp_1$month_start)
...
for (mth in month_starts){
  ...
  ndx_female <- mi_data_grp_1$month_start == mth & mi_data_grp_1$sex == "F"
  ndx_male   <- mi_data_grp_1$month_start == mth & mi_data_grp_1$sex == "M"
  
  # Age adjusted estimates
  mi_std_f <- 
    ageadjust.direct(
      count = mi_data_grp_1$tot_events[ndx_female],
      pop = mi_data_grp_1$tot_py[ndx_female],
      stdpop = mi_data_grp_1$euro_standard_pop[ndx_female]
    )
  
  mi_std_m <- 
    ageadjust.direct(
      count = mi_data_grp_1$tot_events[ndx_male],
      pop = mi_data_grp_1$tot_py[ndx_male],
      stdpop = mi_data_grp_1$euro_standard_pop[ndx_male]
    )
  
  # Add to data frame
  age_std_1[age_std_1$month_start == mth & age_std_1$sex == "Female", 3:6] <- mi_std_f
  age_std_1[age_std_1$month_start == mth & age_std_1$sex == "Male", 3:6]   <- mi_std_m
...
ggplot(age_std_1, aes(x = month_start, y = adj.rate, color = sex, fill = sex)) + 
  ...
  labs(
    x = "Year", 
    y = "Myocaridal infarction: age standardised incidence rate per 1000 person-years", 
    color = "Sex", fill = "Sex"
  )

ggsave(
  file = file.path(output_folder, paste0("03_mi_age_standardised_month_plot_",today(),".pdf")),
  width = 10, height = 8
)
...
for (mth in month_starts){
  ...
  ndx <- mi_data_grp_1_all$month_start == mth
  
  # Age adjusted estimates
  mi_std <- 
    ageadjust.direct(
      count = mi_data_grp_1_all$tot_events[ndx],
      pop = mi_data_grp_1_all$tot_py[ndx],
      stdpop = mi_data_grp_1_all$euro_standard_pop[ndx]
    )
  
  # Add to data frame
  age_std_1_all[age_std_1_all$month_start == mth, 2:5] <- mi_std
...
ggsave(
  file = file.path(output_folder, paste0("03_mi_age_standardised_month_plot_all",today(),".pdf")),
  width = 10, height = 8
)
...
period_4s <- unique(mi_data_grp_4$period_4)
...
for (per in period_4s){
  ...
  ndx_female <- mi_data_grp_4$period_4 == per & mi_data_grp_4$sex == "F"
  ndx_male   <- mi_data_grp_4$period_4 == per & mi_data_grp_4$sex == "M"
  
  # Age adjusted estimate for schizophrenia
  mi_std_sch_f <- 
    ageadjust.direct(
      count = mi_data_grp_4$tot_events_sch[ndx_female],
      pop = mi_data_grp_4$tot_py_sch[ndx_female],
      stdpop = mi_data_grp_4$euro_standard_pop[ndx_female]
    )
  
  mi_std_sch_m <- 
    ageadjust.direct(
      count = mi_data_grp_4$tot_events_sch[ndx_male],
      pop = mi_data_grp_4$tot_py_sch[ndx_male],
      stdpop = mi_data_grp_4$euro_standard_pop[ndx_male]
    )
  
  # Age adjusted estimate for BD
  mi_std_bd_f <- 
    ageadjust.direct(
      count = mi_data_grp_4$tot_events_bd[ndx_female],
      pop = mi_data_grp_4$tot_py_bd[ndx_female],
      stdpop = mi_data_grp_4$euro_standard_pop[ndx_female]
    )
  
  mi_std_bd_m <- 
    ageadjust.direct(
      count = mi_data_grp_4$tot_events_bd[ndx_male],
      pop = mi_data_grp_4$tot_py_bd[ndx_male],
      stdpop = mi_data_grp_4$euro_standard_pop[ndx_male]
    )
  ...
  mi_std_dep_f <- 
    ageadjust.direct(
      count = mi_data_grp_4$tot_events_dep[ndx_female],
      pop = mi_data_grp_4$tot_py_dep[ndx_female],
      stdpop = mi_data_grp_4$euro_standard_pop[ndx_female]
    )
  
  mi_std_dep_m <- 
    ageadjust.direct(
      count = mi_data_grp_4$tot_events_dep[ndx_male],
      pop = mi_data_grp_4$tot_py_dep[ndx_male],
      stdpop = mi_data_grp_4$euro_standard_pop[ndx_male]
    )
  ...
  mi_std_no_smi_f <- 
    ageadjust.direct(
      count = mi_data_grp_4$tot_events_no[ndx_female],
      pop = mi_data_grp_4$tot_py_no[ndx_female],
      stdpop = mi_data_grp_4$euro_standard_pop[ndx_female]
    )
  
  mi_std_no_smi_m <- 
    ageadjust.direct(
      count = mi_data_grp_4$tot_events_no[ndx_male],
      pop = mi_data_grp_4$tot_py_no[ndx_male],
      stdpop = mi_data_grp_4$euro_standard_pop[ndx_male]
    )
  
  # Add to data frame
  age_std_4[age_std_4$period_4 == per & age_std_4$smi == "Schizophrenia" & age_std_4$sex == "Female", 5:8] <- mi_std_sch_f
  age_std_4[age_std_4$period_4 == per & age_std_4$smi == "Schizophrenia" & age_std_4$sex == "Male", 5:8]   <- mi_std_sch_m
  
  age_std_4[age_std_4$period_4 == per & age_std_4$smi == "Bipolar disorder" & age_std_4$sex == "Female", 5:8] <- mi_std_bd_f
  age_std_4[age_std_4$period_4 == per & age_std_4$smi == "Bipolar disorder" & age_std_4$sex == "Male", 5:8]   <- mi_std_bd_m
  
  age_std_4[age_std_4$period_4 == per & age_std_4$smi == "Depression" & age_std_4$sex == "Female", 5:8] <- mi_std_dep_f
  age_std_4[age_std_4$period_4 == per & age_std_4$smi == "Depression" & age_std_4$sex == "Male", 5:8]   <- mi_std_dep_m
  
  age_std_4[age_std_4$period_4 == per & age_std_4$smi == "No SMI" & age_std_4$sex == "Female", 5:8] <- mi_std_no_smi_f
  age_std_4[age_std_4$period_4 == per & age_std_4$smi == "No SMI" & age_std_4$sex == "Male", 5:8] <- mi_std_no_smi_m
...
ggsave(
  file = file.path(output_folder, paste0("03_mi_age_standardised_smi_plot_",today(),".pdf")),
  width = 10, height = 8
)

# 4.4 4-monthly age-standardised incidence rates by SMI (not by sex) ----------
...
for (per in period_4s){
  ...
  ndx <- mi_data_grp_4_all$period_4 == per
  
  # Age adjusted estimate for schizophrenia
  mi_std_sch <- 
    ageadjust.direct(
      count = mi_data_grp_4_all$tot_events_sch[ndx],
      pop = mi_data_grp_4_all$tot_py_sch[ndx],
      stdpop = mi_data_grp_4_all$euro_standard_pop[ndx]
    )
  
  # Age adjusted estimate for BD
  mi_std_bd <- 
    ageadjust.direct(
      count = mi_data_grp_4_all$tot_events_bd[ndx],
      pop = mi_data_grp_4_all$tot_py_bd[ndx],
      stdpop =mi_data_grp_4_all$euro_standard_pop[ndx]
    )
  ...
  mi_std_dep <- 
    ageadjust.direct(
      count = mi_data_grp_4_all$tot_events_dep[ndx],
      pop = mi_data_grp_4_all$tot_py_dep[ndx],
      stdpop = mi_data_grp_4_all$euro_standard_pop[ndx]
    )
  ...
  mi_std_no_smi <- 
    ageadjust.direct(
      count = mi_data_grp_4_all$tot_events_no[ndx],
      pop = mi_data_grp_4_all$tot_py_no[ndx],
      stdpop = mi_data_grp_4_all$euro_standard_pop[ndx]
    )
  
  # Add to data frame
  age_std_4_all[age_std_4_all$period_4 == per & age_std_4_all$smi == "Schizophrenia", 4:7] <- mi_std_sch
  age_std_4_all[age_std_4_all$period_4 == per & age_std_4_all$smi == "Bipolar disorder", 4:7] <- mi_std_bd
  age_std_4_all[age_std_4_all$period_4 == per & age_std_4_all$smi == "Depression", 4:7] <- mi_std_dep
  age_std_4_all[age_std_4_all$period_4 == per & age_std_4_all$smi == "No SMI", 4:7] <- mi_std_no_smi
...
ggsave(
  file = file.path(output_folder, paste0("03_mi_age_standardised_smi_plot_all_",today(),".pdf")),
  width = 10, height = 8
)
...
write.csv(
  age_std_1,
  file = file.path(output_folder, paste0("03_mi_age_standardised_month_",today(),".csv"))
)


write.csv(
  age_std_1_all,
  file = file.path(output_folder, paste0("03_mi_age_standardised_month_all",today(),".csv"))
)

write.csv(
  age_std_4, 
  file = file.path(output_folder, paste0("03_mi_age_standardised_smi_",today(),".csv"))
)

write.csv(
  age_std_4_all, 
  file = file.path(output_folder, paste0("03_mi_age_standardised_smi_all_",today(),".csv"))
)
