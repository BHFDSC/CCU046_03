#
# Derive analysis dataset for myocardial infarction analysis
#
# K. Fleetwood
# 12 July 2024: date started
# 23 Nov 2024: Updated to include people under 40 and over 100
...
cohort_tbl %>%
  mutate(
    age_at_mi  = DATEDIFF(sql("DAY"), date_of_birth, out_ami_primary_date),
    age_at_mi = age_at_mi/365.25
  ) %>%
  ...
  filter(
    ...
    is.na(out_ami_primary_date)|(out_ami_primary_date >= as.Date("2019-11-01")),
    is.na(date_of_death)|is.na(out_ami_primary_date)|(date_of_death >= out_ami_primary_date),
  ) %>%
  mutate(
    ...
    max_age = 
      case_when(
        is.na(out_ami_primary_date) & is.na(date_of_death) ~ age_end_fu,
        !is.na(out_ami_primary_date) ~ age_at_mi,
...
# - Age: 0 to maximum age (as calculated above)
# - Sex: M/F (2 sexes)
...
for (i in 1:50){
  ...
  month_i_sum <- 
    ...
    filter(
      ...
      is.na(out_ami_primary_date)|(out_ami_primary_date >= month_start_i),
    ...
    mutate(
      ...
      sch_date_mi = 
        case_when(
          is.na(out_ami_primary_date) ~ primary_exp_schizophrenia_date,
          is.na(primary_exp_schizophrenia_date) ~ NA,
          primary_exp_schizophrenia_date < out_ami_primary_date ~ primary_exp_schizophrenia_date,
          primary_exp_schizophrenia_date >= out_ami_primary_date ~ NA
        ),
      bd_date_mi = 
        case_when(
          is.na(out_ami_primary_date) ~ primary_exp_bipolar_disorder_date,
          is.na(primary_exp_bipolar_disorder_date) ~ NA,
          primary_exp_bipolar_disorder_date < out_ami_primary_date ~ primary_exp_bipolar_disorder_date,
          primary_exp_bipolar_disorder_date >= out_ami_primary_date ~ NA
        ),
      dep_date_mi = 
        case_when(
          is.na(out_ami_primary_date) ~ primary_exp_depression_date,
          is.na(primary_exp_depression_date) ~ NA,
          primary_exp_depression_date < out_ami_primary_date ~ primary_exp_depression_date,
          primary_exp_depression_date >= out_ami_primary_date ~ NA
        ),
      smi_mi = 
        case_when(
          !is.na(sch_date_mi) ~ "Schizophrenia",
          !is.na(bd_date_mi) ~ "Bipolar disorder",
          !is.na(dep_date_mi) ~ "Depression",
          TRUE ~ "No SMI"
        ), 
      imd_quintile_lab = 
        case_when(
    ...
    mutate(
      ...
      person_years_end = pmin(month_end, date_of_death, out_ami_primary_date),
      person_years_total = DATEDIFF(sql("DAY"), person_years_start, person_years_end) + 1,
      smi_mi_date = 
        case_when(
          smi_mi %in% "No SMI" ~ NA,
          smi_mi %in% "Schizophrenia"~ sch_date_mi,
          smi_mi %in% "Bipolar disorder" ~ bd_date_mi,
          smi_mi %in% "Depression" ~ dep_date_mi
        ),
      smi_start = pmax(month_start, date_of_birth, smi_mi_date),
      person_years_smi = 
        case_when(
      ...
      person_years_no_smi = 
        case_when(
          smi_mi %in% "No SMI" ~ person_years_total,
          TRUE ~ person_years_total - person_years_smi
        ),
      person_years_sch = 
        case_when(
          smi_mi %in% "Schizophrenia" ~ person_years_smi,
          TRUE ~ 0
        ),
      person_years_bd = 
        case_when(
          smi_mi %in% "Bipolar disorder" ~ person_years_smi,
          TRUE ~ 0
        ),
      person_years_dep = 
        case_when(
          smi_mi %in% "Depression" ~ person_years_smi,
          TRUE ~ 0
        ),
      ...
      events_total = 
        case_when(
          !is.na(out_ami_primary_date) & out_ami_primary_date <= month_end & out_ami_primary_date >= date_of_birth ~ 1,
          TRUE                                                             ~ 0
        ),
      events_smi = 
        case_when(
          !is.na(out_ami_primary_date) & out_ami_primary_date <= month_end & out_ami_primary_date >= smi_mi_date & out_ami_primary_date >= date_of_birth ~ 1,
          TRUE                                                                                                   ~ 0
        ),
      events_no_smi = 
        case_when(
          smi_mi %in% "No SMI" ~ events_total,
          TRUE                 ~ events_total - events_smi
        ),
      events_sch = 
        case_when(
          smi_mi %in% "Schizophrenia" ~ events_smi,
          TRUE ~ 0
        ),
      events_bd = 
        case_when(
          smi_mi %in% "Bipolar disorder" ~ events_smi,
          TRUE ~ 0
        ),
      events_dep = 
        case_when(
          smi_mi %in% "Depression" ~ events_smi,
          TRUE ~ 0
        )
    ) %>%
    group_by(
...
saveRDS(month_sum_long, file = file.path(processed_data_folder, "mi_dataset_23Nov2024.rds"))  
