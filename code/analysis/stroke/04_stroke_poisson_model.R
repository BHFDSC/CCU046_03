...
strk_data <- readRDS(file = file.path(processed_data_folder, "stroke_1_dataset.rds"))
strk_data <- strk_data %>% filter(age >= 40 & age <= 100)
...
check_overdispersion(strk_mod_1a)

anova(strk_mod_1a, test = "F") 

strk_mod_1a_coef <- summary(strk_mod_1a)$coef
strk_mod_1a_ci <- confint(strk_mod_1a)

strk_mod_1a_coef <- cbind(strk_mod_1a_coef, strk_mod_1a_ci)
strk_mod_1a_coef <-
  strk_mod_1a_coef %>%
  as.data.frame() %>%
  mutate(
...
anova(strk_mod_1a, strk_mod_1b, test = "F") 
summary(strk_mod_1b)

strk_mod_1b_coef <- summary(strk_mod_1b)$coef
strk_mod_1b_ci <- confint(strk_mod_1b)

strk_mod_1b_coef <- cbind(strk_mod_1b_coef, strk_mod_1b_ci)
strk_mod_1b_coef <-
  strk_mod_1b_coef %>%
  as.data.frame() %>%
  mutate(
...
coefeq <- 
  matrix(
    data = 0,
    nrow = 39, 
    ncol = nrow(strk_mod_1b_coef)
  )
colnames(coefeq) <- row.names(strk_mod_1b_coef)
...
strk_mod_1b_smi_period <- 
  glht(
    strk_mod_1b,
    linfct = coefeq
  )

strk_mod_1b_smi_period_ci <- 
  confint(
    strk_mod_1b_smi_period,
...
strk_mod_1b_smi_period_ci <-
  as.data.frame(strk_mod_1b_smi_period_ci$confint) %>%
  mutate(
    smi = rep(c("Schizophrenia", "Bipolar disorder", "Depression"), each = 13),
    period_4 = rep(levels(strk_data_sub1$period_4), times = 3),
    irr = exp(Estimate),
    irr_low = exp(lwr),
    irr_upp = exp(upr),
    irr_fmt = 
...
check_overdispersion(strk_mod_1a_f)

anova(strk_mod_1a_f, test = "F")

strk_mod_1a_f_coef <- summary(strk_mod_1a_f)$coef
strk_mod_1a_f_ci <- confint(strk_mod_1a_f)

strk_mod_1a_f_coef <- cbind(strk_mod_1a_f_coef, strk_mod_1a_f_ci)
strk_mod_1a_f_coef <-
  strk_mod_1a_f_coef %>%
  as.data.frame() %>%
  mutate(
...
anova(strk_mod_1a_f, strk_mod_1b_f, test = "F") 

strk_mod_1b_f_coef <- summary(strk_mod_1b_f)$coef
strk_mod_1b_f_ci <- confint(strk_mod_1b_f)

strk_mod_1b_f_coef <- cbind(strk_mod_1b_f_coef, strk_mod_1b_f_ci)
strk_mod_1b_f_coef <-
  strk_mod_1b_f_coef %>%
  as.data.frame() %>%
  mutate(
...
strk_mod_1b_f_smi_period <- 
  glht(
    strk_mod_1b_f,
    linfct = coefeq_sex
  )

strk_mod_1b_f_smi_period_ci <- 
  confint(
    strk_mod_1b_f_smi_period,
    calpha = univariate_calpha()
  )
...
strk_mod_1b_f_smi_period_ci <-
  as.data.frame(strk_mod_1b_f_smi_period_ci$confint) %>%
  mutate(
    smi = rep(c("Schizophrenia", "Bipolar disorder", "Depression"), each = 13),
    period_4 = rep(levels(strk_data_sub1$period_4), times = 3),
    irr = exp(Estimate),
...
check_overdispersion(strk_mod_1a_m)

anova(strk_mod_1a_m, test = "F")

strk_mod_1a_m_coef <- summary(strk_mod_1a_m)$coef
strk_mod_1a_m_ci <- confint(strk_mod_1a_m)

strk_mod_1a_m_coef <- cbind(strk_mod_1a_m_coef, strk_mod_1a_m_ci)
strk_mod_1a_m_coef <-
  strk_mod_1a_m_coef %>%
  as.data.frame() %>%
  mutate(
...
anova(strk_mod_1a_m, strk_mod_1b_m, test = "F") 

strk_mod_1b_m_coef <- summary(strk_mod_1b_m)$coef
strk_mod_1b_m_ci <- confint(strk_mod_1b_m)

strk_mod_1b_m_coef <- cbind(strk_mod_1b_m_coef, strk_mod_1b_m_ci)
strk_mod_1b_m_coef <-
  strk_mod_1b_m_coef %>%
  as.data.frame() %>%
  mutate(
...
strk_mod_1b_m_smi_period <- 
  glht(
    strk_mod_1b_m,
    linfct = coefeq_sex
  )

strk_mod_1b_m_smi_period_ci <- 
  confint(
    strk_mod_1b_m_smi_period,
    calpha = univariate_calpha()
  )
...
strk_mod_1b_m_smi_period_ci <-
  as.data.frame(strk_mod_1b_m_smi_period_ci$confint) %>%
  mutate(
    smi = rep(c("Schizophrenia", "Bipolar disorder", "Depression"), each = 13),
    period_4 = rep(levels(strk_data_sub1$period_4), times = 3),
    irr = exp(Estimate),
...
check_overdispersion(strk_mod_2a) 

anova(strk_mod_2a, test = "F") 

strk_mod_2a_coef <- summary(strk_mod_2a)$coef
strk_mod_2a_ci <- confint(strk_mod_2a)

strk_mod_2a_coef <- cbind(strk_mod_2a_coef, strk_mod_2a_ci)
strk_mod_2a_coef <-
  strk_mod_2a_coef %>%
  as.data.frame() %>%
  mutate(
...
anova(strk_mod_2a, strk_mod_2b, test = "F") 

strk_mod_2b_coef <- summary(strk_mod_2b)$coef
strk_mod_2b_ci <- confint(strk_mod_2b)

strk_mod_2b_coef <- cbind(strk_mod_2b_coef, strk_mod_2b_ci)
strk_mod_2b_coef <-
  strk_mod_2b_coef %>%
  as.data.frame() %>%
  mutate(
...
coefeq <- 
  matrix(
    data = 0,
    nrow = 39, 
    ncol = nrow(strk_mod_2b_coef)
  )
colnames(coefeq) <- row.names(strk_mod_2b_coef)
...
strk_mod_2b_smi_period <- 
  glht(
    strk_mod_2b,
    linfct = coefeq
  )

strk_mod_2b_smi_period_ci <- 
  confint(
    strk_mod_2b_smi_period,
    calpha = univariate_calpha()
  )
...
strk_mod_2b_smi_period_ci <-
  as.data.frame(strk_mod_2b_smi_period_ci$confint) %>%
  mutate(
    smi = rep(c("Schizophrenia", "Bipolar disorder", "Depression"), each = 13),
    period_4 = rep(levels(strk_data_sub1$period_4), times = 3),
    irr = exp(Estimate),
...
write.csv(
  strk_mod_1a_coef,
  file = file.path(output_folder, paste0("04_stroke_1_mod_1a_coef_",today(),".csv"))
)

write.csv(
  strk_mod_1b_coef,
  file = file.path(output_folder, paste0("04_stroke_1_mod_1b_coef_",today(),".csv"))
)

write.csv(
  strk_mod_1b_smi_period_ci,
  file = file.path(output_folder, paste0("04_stroke_1_mod_1b_smi_period_",today(),".csv")),
  row.names = FALSE
)
...
write.csv(
  strk_mod_1a_f_coef,
  file = file.path(output_folder, paste0("04_stroke_1_mod_1a_coef_female",today(),".csv"))
)

write.csv(
  strk_mod_1b_f_coef,
  file = file.path(output_folder, paste0("04_stroke_1_mod_1b_coef_female",today(),".csv"))
)

write.csv(
  strk_mod_1b_f_smi_period_ci,
  file = file.path(output_folder, paste0("04_stroke_1_mod_1b_smi_period_female",today(),".csv")),
  row.names = FALSE
)
...
write.csv(
  strk_mod_1a_m_coef,
  file = file.path(output_folder, paste0("04_stroke_1_mod_1a_coef_male",today(),".csv"))
)

write.csv(
  strk_mod_1b_m_coef,
  file = file.path(output_folder, paste0("04_stroke_1_mod_1b_coef_male",today(),".csv"))
)

write.csv(
  strk_mod_1b_m_smi_period_ci,
  file = file.path(output_folder, paste0("04_stroke_1_mod_1b_smi_period_male",today(),".csv")),
  row.names = FALSE
)
...
write.csv(
  strk_mod_2a_coef,
  file = file.path(output_folder, paste0("04_stroke_1_mod_2a_coef_",today(),".csv"))
)

write.csv(
  strk_mod_2b_coef,
  file = file.path(output_folder, paste0("04_stroke_1_mod_2b_coef_",today(),".csv"))
)

write.csv(
  strk_mod_2b_smi_period_ci,
  file = file.path(output_folder, paste0("04_stroke_1_mod_2b_smi_period_",today(),".csv")),
  row.names = FALSE
)
